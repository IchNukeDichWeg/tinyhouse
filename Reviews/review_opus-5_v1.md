...
*Verifier: Sam — I tried to kill this and could not; I produced a wrong PROVEN from the shipped build. 1) QUOTE CHECK (exact). `sed -n '374p' tinyhouse.c | od -c` gives four leading spaces then `if (hdr[1] != tt_mask + 1 || hdr[2] != tt_seed_used) { fclose(f); return -2; }` — character-for-character the item's quote, at the cited line, inside `int th_tt_load(const char *fname)` (tinyhouse.c:368). [...]*
